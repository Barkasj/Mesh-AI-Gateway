...
func LoadConfig(ctx context.Context, configDirPath string) (*Config, error) {
	...
	// If file exists, do a quick check for the expected schema URL.
	// We accept both the Mesh AI Gateway schema URL and the legacy Bifrost schema URL.
	var schema map[string]any
	if err := json.Unmarshal(data, &schema); err != nil {
		return nil, fmt.Errorf("failed to unmarshal schema: %w", err)
	}
	schemaValue, _ := schema["$schema"].(string)
	if !IsAcceptedSchemaURL(schemaValue) {
		// Print warning in yellow ASCII box
		yellowColor := "\033[33m"
		resetColor := "\033[0m"
		message := fmt.Sprintf(
			"config file %s does not include \"$schema\":\"%s\". Use the official schema file to avoid unexpected behavior.",
			absConfigFilePath,
			ConfigSchemaURL(),
		)
	...
	if err := ValidateConfigSchema(data); err != nil {
		logger.Error(
			"config validation failed: %v. You can find the official schema at %s. Some features may not work as expected unless you fix the config file.",
			err,
			ConfigSchemaURL(),
		)
	}
