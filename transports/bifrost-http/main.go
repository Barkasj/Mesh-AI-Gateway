...
func init() {
	...
	// Keep BIFROST_HOST as fallback for backward compatibility.
	defaultHost := os.Getenv("MESH_GATEWAY_HOST")
	if defaultHost == "" {
...
func main() {
	...
	// Printing version
	contentWidth := 59
	versionLine := centerText(Version, contentWidth)
	productLine := centerText(lib.ProductName(), contentWidth)
	taglineLine := centerText("Open Source LLM Gateway", contentWidth)
	projectLine := centerText(lib.ProjectURL(), contentWidth)
...
`, productLine, versionLine, taglineLine, projectLine)
