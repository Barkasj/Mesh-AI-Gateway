...
// GetDefaultConfigDir returns the OS-specific default configuration directory for the gateway.
// This follows standard conventions:
// - Linux/macOS: ~/.config/<config-dir-name>
// - Windows: %APPDATA%\<config-dir-name>
...
func GetDefaultConfigDir(appDir string) string {
	...
	// Explicit override for deployments that want deterministic paths.
	if explicitConfigDir := os.Getenv("MESH_GATEWAY_CONFIG_DIR"); explicitConfigDir != "" {
		return explicitConfigDir
	}

	newDirName := lib.ConfigDirName()
	legacyDirName := lib.LegacyConfigDirName
	...
	default:
		// Linux, macOS and other Unix-like systems: ~/.config/<config-dir-name>
		if homeDir, err := os.UserHomeDir(); err == nil {
	...
	if configRoot != "" {
		configDir := filepath.Join(configRoot, newDirName)
		if legacyPath, ok := maybeUseLegacyConfigPath(configDir, filepath.Join(configRoot, legacyDirName)); ok {
			return legacyPath
		}
		return configDir
	}
	...
	configDir := "./" + newDirName + "-data"
	if legacyPath, ok := maybeUseLegacyConfigPath(configDir, "./"+legacyDirName+"-data"); ok {
		return legacyPath
	}
...
func maybeUseLegacyConfigPath(preferredPath, legacyPath string) (string, bool) {
	if !lib.UseLegacyConfigDirFallback() {
		return "", false
	}
